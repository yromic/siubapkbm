...
export async function saveScores(
  ...
  if (!assessmentId || !scores || !Array.isArray(scores)) {
    throw new AppError(
      "Assessment ID and scores array are required.",
      "ERR_VALIDATION",
      400,
    );
  }
  ...
  try {
    ...
    for (const item of scores) {
      if (!item.student_id) {
        throw new AppError(
          "Missing student_id in input array.",
          "ERR_VALIDATION",
          400,
        );
      }
      ...
      if (!isEmpty) {
        ...
        if (
          scoreNum < assessment.score_min ||
          scoreNum > assessment.score_max
        ) {
          throw new AppError(
      ...
      const enrollment = await db("student_enrollments")
        ...
        .andWhere(function (qb: any) {
          qb.whereNull("withdrawn_at").orWhere(
            db.raw("DATE(withdrawn_at)"),
            ">=",
    ...
    // 4. Audit Log
    const actor = actorId
      ? await db("users").where("id", actorId).first()
      : null;
    for (const item of processedScores) {
    ...
    throw new AppError(
      error instanceof Error
        ? error.message
        : "Database error saving academic scores",
      "ERR_DATABASE",
      500,
    );
  }
...
export async function updateScore(
  ...
  try {
    ...
    if (!existing) {
      throw new AppError(
        `Academic Score with ID ${id} not found.`,
        "ERR_VALIDATION",
        404,
      );
    }
    ...
    throw new AppError(
      error instanceof Error
        ? error.message
        : "Database error updating academic score",
      "ERR_DATABASE",
      500,
    );
  }
...
export async function getStudentAcademicSummary(
  ...
  semesterId: string,
  onlyPublishedAndLocked = false,
) {
  if (!studentId || !academicYearId || !semesterId) {
    throw new AppError(
      "Student ID, Academic Year ID, and Semester ID are required.",
      "ERR_VALIDATION",
      400,
    );
  }
  ...
  try {
    ...
    for (const s of scores) {
      ...
      if (s.score !== null) {
        subjectMap[s.subject_id].total_score = subjectMap[
          s.subject_id
        ].total_score.plus(new Decimal(s.score));
    ...
    const summary = Object.values(subjectMap).map((subj: any) => {
      const avg =
        subj.count > 0 ? subj.total_score.dividedBy(subj.count) : null;
      const roundedAvg = avg
        ? avg.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber()
        : null;
    ...
    throw new AppError(
      error instanceof Error
        ? error.message
        : "Database error getting student academic summary",
      "ERR_DATABASE",
      500,
    );
  }
...
export async function getClassAcademicSummary(
  ...
  if (!classId || !academicYearId || !semesterId) {
    throw new AppError(
      "Class ID, Academic Year ID, and Semester ID are required.",
      "ERR_VALIDATION",
      400,
    );
  }
  ...
  try {
    ...
    for (const score of scores) {
      if (score.score !== null && score.score !== undefined) {
        ...
        scoresByAssessment[score.assessment_id][score.student_id] = Number(
          score.score,
        );
      }
    }
    ...
    const allStudentsMap: Record<
      string,
      { student_id: string; full_name: string; nisn: string; scores: number[] }
    > = {};
    ...
    for (const assessment of assessments) {
      ...
      const roster = await db("student_enrollments")
        ...
        .where(
          db.raw("DATE(student_enrollments.enrolled_at)"),
          "<=",
          assessmentDate,
        )
        .andWhere(function (qb: any) {
          qb.whereNull("student_enrollments.withdrawn_at").orWhere(
            db.raw("DATE(student_enrollments.withdrawn_at)"),
            ">=",
            assessmentDate,
          );
        })
    ...
    const student_summaries = Object.values(allStudentsMap).map((student) => {
      const average_score =
        student.scores.length > 0
          ? parseFloat(
              (
                student.scores.reduce((sum, val) => sum + val, 0) /
    ...
    throw new AppError(
      error instanceof Error
        ? error.message
        : "Database error getting class academic summary",
      "ERR_DATABASE",
      500,
    );
  }
...
export async function getClassAcademicAverages(
  ...
  for (const cls of classes) {
    ...
    if (Number(studentCountRes?.count || 0) < 3) {
      continue;
    }
    ...
    const avg = await db("academic_scores")
      ...
      .join(
        "student_enrollments",
        "academic_scores.student_enrollment_id",
...
export async function getStudentsWithoutScores(
  ...
  Array<{ id: string; full_name: string; nisn: string; reason: string }>
> {
...
/**
 * Returns the canonical student watchlist for a semester.
 * The business rules for missing academic scores and missing culture scores stay in their
 * respective domain services; this helper only orchestrates and deduplicates the results.
 */
export async function getStudentWatchlistEntries(
  semesterId: string,
  limit = 50,
): Promise<
  Array<{ id: string; full_name: string; nisn: string; reasons: string[] }>
> {
  if (!semesterId) return [];
  ...
  const combined: Record<
    string,
    { id: string; full_name: string; nisn: string; reasons: string[] }
  > = {};

  for (const student of [...studentsWithNoScores, ...studentsWithNoCulture]) {
    if (!combined[student.id]) {
      combined[student.id] = {
        id: student.id,
        full_name: student.full_name,
        nisn: student.nisn,
    ...
    combined[student.id].reasons.push(student.reason);
  }
