...
export default function PromotionPage() {
  ...
  return <ResponsiveContainer className="space-y-6">
    ...
    <section className="rounded-2xl border border-zinc-200 bg-surface-1 p-5 dark:border-zinc-800">
      ...
      <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300">Khusus kenaikan tahun ajaran dari semester akhir ke semester awal. Bukan rollover Ganjil → Genap dalam tahun ajaran yang sama.</div>
      {initialLoading && <div className="mb-4 rounded-xl bg-zinc-50 p-3 text-sm text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">Memuat tahun ajaran, semester, dan kelas...</div>}
      {initialError && <div role="alert" className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300"><span>{initialError}</span><button type="button" disabled={initialLoading} onClick={() => void loadInitialData()} className="rounded-lg border border-red-300 px-3 py-1.5 font-semibold disabled:opacity-50 dark:border-red-800">Coba Lagi</button></div>}
      {classLoadWarning && <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300">Daftar kelas gagal dimuat: {classLoadWarning} Target kelas untuk tindakan manual naik kelas belum tersedia, tetapi pilihan periode tetap dapat digunakan.</div>}
      ...
      <button type="button" onClick={() => void runPreview()} disabled={loading || initialLoading || !sourceSemesterId || !targetSemesterId} className="mt-5 rounded-xl bg-brand-emerald-600 px-5 py-2.5 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 inline-flex items-center gap-2">
        {loading && <Loader2 className="w-4 h-4 animate-spin" />}
        {loading ? "Memproses..." : "Pratinjau Kenaikan Kelas"}
      </button>
    ...
    {summary && <section className="rounded-2xl border border-zinc-200 bg-surface-1 p-5 dark:border-zinc-800">
      <h2 className="mb-4 text-lg font-bold">{result ? "Hasil Pelaksanaan" : "Ringkasan Pratinjau"}</h2>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-5">{summaryKeys.map((key) => <div key={key} className="rounded-xl bg-zinc-50 p-3 dark:bg-zinc-800"><div className="text-xs font-semibold text-zinc-500">{formatPromotionSummaryLabel(key)}</div><div className="text-2xl font-bold">{String(summary[key as keyof typeof summary] ?? 0)}</div></div>)}</div>
    </section>}

    {preview && <section className="space-y-4">
      <div className={`rounded-xl border p-4 ${preview.can_execute ? "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300" : "border-red-200 bg-red-50 text-red-800 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300"}`}><strong>{preview.can_execute ? "Siap diproses" : "Belum dapat diproses"}</strong>{preview.global_blockers.map((b, i) => <p key={`${b.type}-${i}`} className="mt-1 text-sm">{localizePromotionMessage(b.message)}</p>)}</div>
      <div className="overflow-x-auto rounded-2xl border border-zinc-200 bg-surface-1 dark:border-zinc-800"><table className="min-w-full text-sm"><thead className="bg-zinc-50 text-left dark:bg-zinc-800"><tr><th className="p-3">Siswa</th><th className="p-3">Kelas asal</th><th className="p-3">Rekomendasi</th><th className="p-3">Tindakan Manual</th><th className="p-3">Target kelas</th><th className="p-3">Kendala</th></tr></thead><tbody>{preview.students.map((student) => {
      ...
      <button type="button" disabled={loading || !preview.can_execute} onClick={() => setConfirmOpen(true)} className="rounded-xl bg-red-600 px-5 py-3 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 inline-flex items-center gap-2">
        {loading && <Loader2 className="w-4 h-4 animate-spin" />}
    ...
    {confirmOpen && preview && <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="promotion-confirm-title" onKeyDown={(e) => { if (e.key === "Escape") setConfirmOpen(false); }}><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-surface-1 p-6 shadow-xl"><h2 id="promotion-confirm-title" className="text-xl font-bold">Konfirmasi Kenaikan Kelas</h2><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><Summary label="Periode Sumber" value={sourcePeriod} wide /><Summary label="Periode Target" value={targetPeriod} wide /><Summary label="Naik Kelas" value={preview.counts.promoted} /><Summary label="Tinggal Kelas" value={preview.counts.repeated} /><Summary label="Lulus" value={preview.counts.graduated} /><Summary label="Pindah" value={preview.counts.transferred} /><Summary label="Tidak Aktif" value={preview.counts.inactive} /><Summary label="Keluar" value={preview.counts.left} /><Summary label="Kendala" value={preview.global_blockers.length + (preview.counts.unresolved || 0)} /><Summary label="Perubahan Manual" value={manualOverrideCount} /></dl><p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300">Proses ini akan memperbarui riwayat kelas siswa dan tidak dapat dibatalkan secara otomatis.</p><div className="mt-6 flex justify-end gap-3"><button type="button" disabled={loading} onClick={() => setConfirmOpen(false)} className="rounded-xl border border-zinc-300 px-4 py-2 dark:border-zinc-700">Batal</button><button type="button" disabled={loading} onClick={() => void execute()} className="rounded-xl bg-red-600 px-4 py-2 font-semibold text-white disabled:opacity-50 inline-flex items-center gap-2">{loading && <Loader2 className="w-4 h-4 animate-spin" />}{loading ? "Memproses..." : "Ya, Proses"}</button></div></div></div>}
  </ResponsiveContainer>;
}

function Summary({ label, value, wide = false }: { label: string; value: string | number; wide?: boolean }) {
  return <div className={`rounded-xl bg-zinc-50 p-3 dark:bg-zinc-800 ${wide ? "col-span-2" : ""}`}><dt className="text-xs text-zinc-500">{label}</dt><dd className="mt-1 font-semibold">{value}</dd></div>;
}
