...
export default function PromotionPage() {
  ...
  return <ResponsiveContainer className="space-y-6">
    ...
    <section className="rounded-[20px] border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-[#171717]">
      ...
      <button type="button" onClick={() => void runPreview()} disabled={loading || initialLoading || !sourceSemesterId || !targetSemesterId} className="mt-5 rounded-[12px] bg-[#468432] px-5 py-2.5 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 inline-flex items-center gap-2">
    ...
    {preview && <section className="space-y-4">
      ...
      <button type="button" disabled={loading || !preview.can_execute} onClick={() => setConfirmOpen(true)} className="rounded-[12px] bg-red-600 px-5 py-3 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 inline-flex items-center gap-2">
        {loading && <Loader2 className="w-4 h-4 animate-spin" />}
