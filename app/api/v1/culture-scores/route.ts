...
import { saveCultureScores, listCultureScoresByWeek } from "@/lib/services/cultureScoreService";
  ...
  return withAuth(req, async (req) => {
    return withRole(["administrator", "teacher"], req, async () => {
      try {
        ...
        if (!week_start_date || !class_id) {
          return errorResponse("week_start_date (YYYY-MM-DD) and class_id query parameters are required.", "ERR_VALIDATION", 400);
        }
        ...
        const result = await listCultureScoresByWeek(week_start_date, class_id);
