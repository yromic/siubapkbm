...
import { getAcademicCompleteness } from '@/lib/services/completenessService';
  ...
  return withAuth(req, async (req) => {
    return withRole(['administrator'], req, async () => {
      try {
        ...
        const percentage = await getAcademicCompleteness(class_id, academic_year_id, semester_id);
        return successResponse({ percentage }, 'Academic completeness calculated.');
      } catch (error) {
