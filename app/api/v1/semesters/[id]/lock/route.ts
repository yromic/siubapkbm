...
import { lockSemester } from '@/lib/services/semesterService';
  ...
  return withAuth(req, async (req) => {
    return withRole(['administrator'], req, async () => {
      try {
        ...
        const result = await lockSemester(id, actorId);
        return successResponse(result, 'Semester locked successfully.');
      } catch (error) {
