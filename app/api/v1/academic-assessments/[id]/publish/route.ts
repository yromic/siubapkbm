...
import { getAssessmentById, publishAssessment } from '@/lib/services/assessmentService';
    ...
    return withRole(['administrator', 'teacher'], req, async () => {
      try {
        ...
        if (actor && actor.role === 'teacher' && assessment.teacher_user_id !== actorId) {
          return errorResponse('You do not have permission to publish this assessment.', 'ERR_FORBIDDEN', 403);
        }

        const result = await publishAssessment(id);
        return successResponse(result, 'Academic assessment published successfully.');
      } catch (error) {
