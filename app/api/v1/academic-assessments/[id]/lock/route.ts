...
import { getAssessmentById, lockAssessment } from '@/lib/services/assessmentService';
    ...
    return withRole(['administrator', 'teacher'], req, async () => {
      try {
        ...
        if (actor && actor.role === 'teacher' && assessment.teacher_user_id !== actorId) {
          return errorResponse('You do not have permission to lock this assessment.', 'ERR_FORBIDDEN', 403);
        }

        const result = await lockAssessment(id, actorId);
        return successResponse(result, 'Academic assessment locked successfully.');
      } catch (error) {
