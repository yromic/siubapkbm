...
import { getStudentProgressDashboard } from '@/lib/services/dashboardService';
  ...
  return withAuth(req, async (req) => {
    return withRole(['administrator', 'admin', 'teacher'], req, async () => {
      try {
        ...
        const data = await getStudentProgressDashboard(id);
        return successResponse(data, 'Student progress dashboard retrieved.');
      } catch (error) {
